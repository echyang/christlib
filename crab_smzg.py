...
if __name__ == "__main__":
	...
	params = {
			'url':'http://www.smzg.org/home/%E5%A9%9A%E5%A7%BB%E8%89%AF%E8%8D%AF-love-potion-letters/',
			'fields':[
				{'name':'author', 'select':'#post-1709 > div > div > a', 'regex':''},	
				{'name':'title', 'select':'#post-1709 header h1 a', 'regex':''},	
				{'name':'pages', 'select':'#post-1709 div ol li span a', 'regex':'', 'url':True, 'multi':True}	
			],
			'charset':'utf8'
		}	
	...
	child_params = {
			'url':'',
			'fields':[
				{'name':'content', 'select':'div.entry-content', 'regex':'', 'number':0, 'content':True}	
			],
			'charset':'utf8'
		}	
...
'''
	...
	for page_data in pages_data:	
		...
		f = open('%s/%02d.html' % (index_data['title'], i),'w')
		f.write(text_start+text_content+text_end)
		f.close()
